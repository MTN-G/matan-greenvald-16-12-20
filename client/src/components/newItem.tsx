...
const NewItem: React.FC<NewItemProps> = ({ setToggle, items, stores, labels, getItems, getStores, getLabels }) => {
    ...
    async function addItem(item: Partial<Item>) {
        try {
            ...
            else if (!item.store) setError ("please select store")
            else {
            ...
            <StyledForm>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <StyledSelect onChange={(e: any) => {
                        const tempItem = newItem;
                        tempItem!.store = stores.find(store => store.name === e.target.value)?.id!
                        setNewItem(tempItem);
                ...
                </div><span>{error.length > 1 && <StyledDiv repeatFormula="1fr" style={{ backgroundColor: "red", marginBottom: "60%"}}>{error}</StyledDiv>}</span>
                <div style={{ display: "flex", flexDirection: "column" }}>
                    
                    <StyledSelect onChange={(e: any) => {
                        const tempItem = newItem;
                        tempItem!.labels = [labels.find(label => label.name === e.target.value)?.id!];
                        setNewItem(tempItem)
